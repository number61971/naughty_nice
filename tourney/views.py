...
def common_page_data(req, tournament_id):
    if int(tournament_id):
        ...
        id = int(req.session.get('tournament_id'))
        if id:
            ...
            tournament_id = 0
            masthead = "Battle Points Tournament Manager"
            tagline = 'Making my life easier!'
            notes_count = 0
...
def tournament_update(req):
    ...
    if form.is_valid():
        ...
        tournament.save()
        # success redirect
        return redirect('/tourney/players/%s' % tournament.id)
    else:
...
def tournament_notes(req, tournament_id):
    ...
    data['action'] = '/tourney/tournament/%s/notes/new' % tournament_id
...
def create_tournament_note(req, tournament_id):
    ...
    if form.is_valid():
        ...
        # success redirect
        return redirect ('/tourney/tournament/%s/notes' % tournament_id)
    else:
...
def players(req):
    ...
    if not tournament_id:
        try:
            ...
            return redirect('/tourney/tournament/0')
    return redirect('/tourney/players/%s' % tournament_id)
...
def player_remove_from_tournament(req, tournament_id, tplayer_id):
    ...
    sql = "delete from armylistunit where armylist_id=%s;" % tplayer.armylist.id
    execute_sql(sql)
    sql = "delete from armylist where id=%s;" % tplayer.armylist.id
    execute_sql(sql)
    sql = "delete from tournamentplayer where id=%s;" % tplayer.id
    execute_sql(sql)
    return redirect('/tourney/players/%s' % tournament_id)
...
def player_notes(req, tournament_id, player_id):
    ...
    data['form'] = NoteForm()
    data['valid_form'] = True
    data['action'] = '/tourney/players/%s/%s/notes/new' % (
...
def create_player_note(req, tournament_id, player_id):
    ...
    if form.is_valid():
        ...
        player_note.save()
        # success redirect
        return redirect ('/tourney/players/%s/%s/notes' % (
            tournament_id, player_id))
    else:
        # return form with errors
        data = common_page_data(req, tournament_id)
...
def army_list_update(req):
    ...
    if armylist_form.is_valid():
        ...
        armylist.save()
        # success redirect
        return redirect('/tourney/players/%s/%s/army_list' % (
        ...
        if list_data[slot]:
            ...
            for unit in list_data[slot]:
                ...
                if unit['details']:
                    ...
                    details = ''
                if deletable_units:
                    delete_action = '<img id="delete_unit_%s" class="action_delete" src="/tourney/static/img/action_delete.png" alt="Delete" title="Delete"/> ' % unit['id']
                else:
                    delete_action = ''
...
def games(req):
    ...
    if not round_number:
        try:
            ...
            round_number = 0
    return redirect('/tourney/games/%s/%s' % (tournament_id,round_number))
    ...
    try:
        ...
        for g in games:
            ...
            data = {
              ...
              'player1_battle_points': g.player1_battle_points(),
              ...
              'player2_battle_points': g.player2_battle_points(),
...
def games_new_round(req, tournament_id):
    ...
    new_round.save()
    return redirect('/tourney/games/%s/%s/pairings' % (
...
def fetch_player_game_scores(req, game_id, player_num):
    game = Game.objects.get(id=game_id)
    if player_num in (1,'1'):
        player = game.player1
        primary_objective_points = game.player1_primary_objective_points
        secondary_objective_points = game.player1_secondary_objective_points
        tertiary_objective_points = game.player1_tertiary_objective_points
        bonus_points = game.player1_bonus_points
        penalty_points = game.player1_penalty_points
        naughty_points = game.player1_naughty_points
        nice_points = game.player1_nice_points
    else:
        player = game.player2
        primary_objective_points = game.player2_primary_objective_points
        secondary_objective_points = game.player2_secondary_objective_points
        tertiary_objective_points = game.player2_tertiary_objective_points
        bonus_points = game.player2_bonus_points
        penalty_points = game.player2_penalty_points
        naughty_points = game.player2_naughty_points
        nice_points = game.player2_nice_points
    response = {'response':'success',
                'data':{'player_name':player.player.fullname(),
                        'player_army':player.armylist.race.abbrev,
                        'primary_objective_points':primary_objective_points,
                        'secondary_objective_points':secondary_objective_points,
                        'tertiary_objective_points':tertiary_objective_points,
                        'bonus_points':bonus_points,
                        'penalty_points':penalty_points,
                        'naughty_points':naughty_points,
...
def game_update(req):
    idgame = req.POST['game_id']
    game = Game.objects.get(id=idgame)
    player = int(req.POST['player_num'])
    if player == 1:
        game.player1_primary_objective_points = int(req.POST['primary_objective_points'])
        game.player1_secondary_objective_points = int(req.POST['secondary_objective_points'])
        game.player1_tertiary_objective_points = int(req.POST['tertiary_objective_points'])
        game.player1_bonus_points = int(req.POST['bonus_points'])
        game.player1_penalty_points = int(req.POST['penalty_points'])
        game.player1_naughty_points = int(req.POST['naughty_points'])
        game.player1_nice_points = int(req.POST['nice_points'])
        game.save()
        battle_points = game.player1_battle_points()
    elif player == 2:
        game.player2_primary_objective_points = int(req.POST['primary_objective_points'])
        game.player2_secondary_objective_points = int(req.POST['secondary_objective_points'])
        game.player2_tertiary_objective_points = int(req.POST['tertiary_objective_points'])
        game.player2_bonus_points = int(req.POST['bonus_points'])
        game.player2_penalty_points = int(req.POST['penalty_points'])
        game.player2_naughty_points = int(req.POST['naughty_points'])
        game.player2_nice_points = int(req.POST['nice_points'])
        game.save()
        battle_points = game.player2_battle_points()
    response = {'response':'success',
                'data':{'game_id':idgame,
                        'player_num':player,
...
def games_delete_round(req, tournament_id, round_number):
    ...
    return redirect('/tourney/games/%s/%s' % (tournament_id,prev_round))
...
def game_notes(req, tournament_id, game_id):
    ...
    data['form'] = NoteForm()
    data['valid_form'] = True
    data['action'] = '/tourney/games/%s/%s/notes/new' % (
...
def create_game_note(req, tournament_id, game_id):
    ...
    if form.is_valid():
        ...
        game_note.save()
        # success redirect
        return redirect ('/tourney/games/%s/%s/notes' % (
            tournament_id, game_id))
    else:
        # return form with errors
        data = common_page_data(req, tournament_id)
...
def standings(req):
    ...
    if not round_number:
        try:
            ...
            round_number = 0
    return redirect('/tourney/standings/%s/%s' % (
        tournament_id, round_number))
...
def standings_list(req, tournament_id, round_number):
    ...
    for s in standings['list']:
        ...
        for g in games_data:
            ...
            if g.player1.id == tp_data['tplayer_id']:
                opp = g.player2
                opp_bps = g.player2_battle_points()
                bps = g.player1_battle_points()
            else:
                opp = g.player1
                opp_bps = g.player1_battle_points()
                bps = g.player2_battle_points()
            if bps > opp_bps:
                result = '%s BPs > %s BPs (W)' % (bps, opp_bps)
            elif bps < opp_bps:
                result = '%s BPs < %s BPs (L)' % (bps, opp_bps)
            else:
                result = '%s BPs = %s BPs (D)' % (bps, opp_bps)
            games.append({
                'round': g.round.round,
                'table': g.table,
                'opponent': opp,
    ...
    naughtiest = [p for p in players[:] if p['results']['naughty_points'] != 0]
    naughtiest.sort(naughty_score_sort)

    nicest = [p for p in players[:] if p['results']['nice_points'] != 0]
    nicest.sort(nice_score_sort)
    ...
    data['naughtiest'] = naughtiest[:3]
    data['nicest'] = nicest[:3]
    return render_to_response('standings.html', data)

def naughty_score_sort(a,b):
    if a['results']['naughty_points'] < b['results']['naughty_points']:
        return -1
    elif a['results']['naughty_points'] > b['results']['naughty_points']:
        return 1
    elif a['results']['nice_points'] < b['results']['nice_points']:
        return -1
    elif a['results']['nice_points'] > b['results']['nice_points']:
        return 1
    elif a['results']['battle_points'] > b['results']['battle_points']:
        return -1
    elif a['results']['battle_points'] < b['results']['battle_points']:
        return 1
    elif a['results']['primary_objective_points'] > b['results']['primary_objective_points']:
        return -1
    elif a['results']['primary_objective_points'] < b['results']['primary_objective_points']:
        return 1
    elif a['results']['secondary_objective_points'] > b['results']['secondary_objective_points']:
        return -1
    elif a['results']['secondary_objective_points'] < b['results']['secondary_objective_points']:
        return 1
    elif a['results']['tertiary_objective_points'] > b['results']['tertiary_objective_points']:
        return -1
    elif a['results']['tertiary_objective_points'] < b['results']['tertiary_objective_points']:
        return 1
    elif a['results']['bonus_points'] > b['results']['bonus_points']:
        return -1
    elif a['results']['bonus_points'] < b['results']['bonus_points']:
        return 1
    elif a['results']['penalty_points'] > b['results']['penalty_points']:
        return -1
    elif a['results']['penalty_points'] < b['results']['penalty_points']:
        return 1
    else:
...
def nice_score_sort(a,b):
    if a['results']['nice_points'] > b['results']['nice_points']:
        return -1
    elif a['results']['nice_points'] < b['results']['nice_points']:
        return 1
    elif a['results']['naughty_points'] > b['results']['naughty_points']:
...
def appearance(req):
    ...
    if not tournament_id:
        try:
            ...
            return redirect('/tourney/tournament/0')
    return redirect('/tourney/appearance/%s' % tournament_id)
...
def appearance_update(req):
    ...
    if form.is_valid():
        ...
        appearance.save()
        # success redirect
        return redirect('/tourney/appearance/%s' % tplayer.tournament.id)
    else:
        # return edit form with errors
...
def appearance_notes(req, tournament_id, tplayer_id):
    ...
    data['form'] = NoteForm()
    data['valid_form'] = True
    data['action'] = '/tourney/appearance/%s/%s/notes/new' % (
...
def create_appearance_note(req, tournament_id, tplayer_id):
    ...
    if form.is_valid():
        ...
        # success redirect
        return redirect ('/tourney/appearance/%s/%s/notes' % (
            tournament_id, tplayer_id))
    else:
        # return form with errors
        if tplayer.armylist.description:
...
def sportsmanship(req):
    ...
    if not tournament_id:
        try:
            ...
            return redirect('/tourney/tournament/0')
    return redirect('/tourney/sports/%s' % tournament_id)
...
def sportsmanship_update_discretionary(req):
    ...
    if form.is_valid():
        ...
        tplayer.save()
        # success redirect
        return redirect('/tourney/sports/%s' % tplayer.tournament.id)
    else:
        # return edit form with errors
        data = common_page_data(req, tplayer.tournament.id)
        data['form'] = form
...
def sportsmanship_new_blackmark(req):
    ...
    if form.is_valid():
        ...
        blackmark.save()
        # success redirect
        return redirect('/tourney/sports/%s/%s/blackmarks' % (
...
def sportsmanship_notes(req, tournament_id, tplayer_id):
    ...
    data['form'] = NoteForm()
    data['valid_form'] = True
    data['action'] = '/tourney/sports/%s/%s/notes/new' % (
            tournament_id, tplayer_id)
...
def create_sportsmanship_note(req, tournament_id, tplayer_id):
    ...
    if form.is_valid():
        ...
        player_note.save()
        # success redirect
        return redirect ('/tourney/sports/%s/%s/notes' % (
            tournament_id, tplayer_id))
    else:
        # return form with errors
        data = common_page_data(req, tournament_id)
...
def overall(req):
    ...
    if not tournament_id:
        try:
            ...
            return redirect('/tourney/tournament/0')
    return redirect('/tourney/overall/%s' % tournament_id)
