...
class Tournament(models.Model):
    ...
    ranking_method = models.CharField('Ranking Method', default='battle',
...
class TournamentPlayer(models.Model):
    ...
    def results(self, accelerated_swiss_weighted=False, highest_round=None):
        record = {'W':0, 'L':0, 'D':0, 'battle_points':0, 
                  'primary_objective_points':0,
                  'secondary_objective_points':0,
                  'tertiary_objective_points':0,
                  'bonus_points':0, 'penalty_points':0,
                  'naughty_points':0, 'nice_points':0}
        for game in self.games(highest_round=highest_round):
            if game.player1.player.id == self.player.id:
                player_bps = game.player1_battle_points()
                player_primary_objective_points = game.player1_primary_objective_points
                player_secondary_objective_points = game.player1_secondary_objective_points
                player_tertiary_objective_points = game.player1_tertiary_objective_points
                player_bonus_points = game.player1_bonus_points
                player_penalty_points = game.player1_penalty_points
                player_naughty_points = game.player1_naughty_points
                player_nice_points = game.player1_nice_points
                opp_bps = game.player2_battle_points()
                opp_primary_objective_points = game.player2_primary_objective_points
                opp_secondary_objective_points = game.player2_secondary_objective_points
                opp_tertiary_objective_points = game.player2_tertiary_objective_points
                opp_bonus_points = game.player2_bonus_points
                opp_penalty_points = game.player2_penalty_points
                opp_naughty_points = game.player2_naughty_points
                opp_nice_points = game.player2_nice_points
            elif game.player2.player.id == self.player.id:
                player_bps = game.player2_battle_points()
                player_primary_objective_points = game.player2_primary_objective_points
                player_secondary_objective_points = game.player2_secondary_objective_points
                player_tertiary_objective_points = game.player2_tertiary_objective_points
                player_bonus_points = game.player2_bonus_points
                player_penalty_points = game.player2_penalty_points
                player_naughty_points = game.player2_naughty_points
                player_nice_points = game.player2_nice_points
                opp_bps = game.player1_battle_points()
                opp_primary_objective_points = game.player1_primary_objective_points
                opp_secondary_objective_points = game.player1_secondary_objective_points
                opp_tertiary_objective_points = game.player1_tertiary_objective_points
                opp_bonus_points = game.player1_bonus_points
                opp_penalty_points = game.player1_penalty_points
                opp_naughty_points = game.player1_naughty_points
                opp_nice_points = game.player1_nice_points
            else:
                raise Exception('ERROR: Wrong games returned for this player.')

            record['battle_points'] += player_bps
            record['primary_objective_points'] += player_primary_objective_points
            record['secondary_objective_points'] += player_secondary_objective_points
            record['tertiary_objective_points'] += player_tertiary_objective_points
            record['bonus_points'] += player_bonus_points
            record['penalty_points'] += player_penalty_points
            record['naughty_points'] += player_naughty_points
            record['nice_points'] += player_nice_points

            if player_bps > opp_bps:
                record['W'] += 1
            elif player_bps < opp_bps:
                record['L'] += 1
            else:
                record['D'] += 1
        ...
        if accelerated_swiss_weighted and self.accelerated_swiss_pairing_bonus:
            if self.tournament.ranking_method == 'record':
                ...
                record['battle_points'] += 30 # maximum BPs in a game
...
class Game(models.Model):
    ...
    player1_primary_objective_points = models.IntegerField(
            'Player 1 Primary Objective Points', default=0)
    player1_secondary_objective_points = models.IntegerField(
            'Player 1 Secondary Objective Points', default=0)
    player1_tertiary_objective_points = models.IntegerField(
            'Player 1 Tertiary Objective Points', default=0)
    player1_bonus_points = models.IntegerField(
            'Player 1 Bonus Points', default=0)
    player1_penalty_points = models.IntegerField(
            'Player 1 Penalty Points', default=0)
    player1_naughty_points = models.IntegerField(
            'Player 1 Naughty Points', default=0)
    player1_nice_points = models.IntegerField(
            'Player 1 Nice Points', default=0)
    player2 = models.ForeignKey('TournamentPlayer', related_name='player2_id')
    player2_primary_objective_points = models.IntegerField(
            'Player 2 Primary Objective Points', default=0)
    player2_secondary_objective_points = models.IntegerField(
            'Player 2 Secondary Objective Points', default=0)
    player2_tertiary_objective_points = models.IntegerField(
            'Player 2 Tertiary Objective Points', default=0)
    player2_bonus_points = models.IntegerField(
            'Player 2 Bonus Points', default=0)
    player2_penalty_points = models.IntegerField(
            'Player 2 Penalty Points', default=0)
    player2_naughty_points = models.IntegerField(
            'Player 2 Naughty Points', default=0)
    player2_nice_points = models.IntegerField(
            'Player 2 Nice Points', default=0)
    table = models.IntegerField('Table', blank=True, null=True)

    notes = models.ManyToManyField('Note', through='GameNote',
                                   symmetrical=False)

    def player1_battle_points(self):
        return (self.player1_primary_objective_points
                + self.player1_secondary_objective_points
                + self.player1_tertiary_objective_points
                + self.player1_bonus_points
                + self.player1_penalty_points
                + self.player1_naughty_points
                + self.player1_nice_points)

    def player2_battle_points(self):
        return (self.player2_primary_objective_points
                + self.player2_secondary_objective_points
                + self.player2_tertiary_objective_points
                + self.player2_bonus_points
                + self.player2_penalty_points
                + self.player2_naughty_points
                + self.player2_nice_points)

    def result(self):
        player1_battle_points = self.player1_battle_points()
        player2_battle_points = self.player2_battle_points()
        if player1_battle_points > self.player2_battle_points:
            return self.player1
        elif player1_battle_points < self.player2_battle_points:
            return self.player2
        else:
            return None
...
def standings_by_record(p1, p2):
    ...
    if p1['results']['battle_points'] < p2['results']['battle_points']:
        return 1
    if p1['results']['primary_objective_points'] > p2['results']['primary_objective_points']:
        return -1
    if p1['results']['primary_objective_points'] < p2['results']['primary_objective_points']:
        return 1
    if p1['results']['secondary_objective_points'] > p2['results']['secondary_objective_points']:
        return -1
    if p1['results']['secondary_objective_points'] < p2['results']['secondary_objective_points']:
        return 1
    if p1['results']['tertiary_objective_points'] > p2['results']['tertiary_objective_points']:
        return -1
    if p1['results']['tertiary_objective_points'] < p2['results']['tertiary_objective_points']:
        return 1
    if p1['results']['nice_points'] > p2['results']['nice_points']:
        return -1
    if p1['results']['nice_points'] < p2['results']['nice_points']:
        return 1
    if p1['results']['bonus_points'] > p2['results']['bonus_points']:
        return -1
    if p1['results']['bonus_points'] < p2['results']['bonus_points']:
        return 1
    if p1['results']['naughty_points'] > p2['results']['naughty_points']:
        return -1
    if p1['results']['naughty_points'] < p2['results']['naughty_points']:
        return 1
    if p1['results']['penalty_points'] > p2['results']['penalty_points']:
        return -1
    if p1['results']['penalty_points'] < p2['results']['penalty_points']:
        return 1
    return 0
...
class TournamentForm(forms.Form):
    ...
    end_date = forms.DateField(label='End Date', required=False,
            widget=forms.DateInput(format='%m/%d/%Y'))
    ranking_method = forms.ChoiceField(label='Ranking Method', initial='battle',
